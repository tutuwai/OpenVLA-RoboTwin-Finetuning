...
基于 RoboTwin 仿真平台与 OpenVLA-oft 的双臂机器人端到端控制微调仓库。  
仓库覆盖完整工程链路：数据采集、数据转换、RLDS 构建、分布式训练、LoRA 合并与评测。

## 项目简介

本仓库在 RoboTwin 仿真环境中复现 OpenVLA-oft 微调流程，提供从专家数据采集到 7B VLA 分布式训练与评测的完整闭环，并包含末端 action-chunk 数据覆盖问题的修复实现。

## 技术方案概述

### 系统实现与实验设置

VLA 是基于多模态大模型的机器人端到端控制模型，常用于 manipulation 任务。  
本仓库基于 OpenVLA 开源代码实现 OpenVLA-oft 训练与评测流程，包含仿真数据生成、数据转换、分布式训练与评测模块。
...
- 端到端流程：数据采集 -> 数据转换 -> RLDS 构建 -> 分布式训练 -> LoRA 合并 -> 仿真评测
- 训练方案：面向 7B VLA 的 LoRA + 多卡 FSDP 组合
- 多模态输入：支持多视角图像与 proprio 接入 OpenVLA-oft
- 数据修复：修复轨迹末尾样本裁剪问题，提升末段状态覆盖
- 文档支持：提供流程图与 shape 流图（`doc/*.d2`）
...
> 若算力有限，可先运行约 100 step 验证流程可用性；完整收敛主要受训练资源限制。

## 方法概览

- **目标**：评估 VLA 在双臂 manipulation 任务中的端到端可行性与稳定性。
- **路径**：先完成数据闭环，再完成训练闭环，最后定位并修复末段失败问题。
- **难点**：7B 模型显存压力、数据格式对齐、action chunk 监督覆盖不足。
- **改动**：完善 FSDP 训练链路，修复 `traj_transforms.py` 末尾样本逻辑。
...
如需快速验证评测链路，可直接使用公开权重执行 eval，无需等待完整训练结束。
